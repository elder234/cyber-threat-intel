...
export default async function dashboardRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get('/stats', { preHandler: [app.authenticate], schema: { tags: ['dashboard'] } },
  ...
  app.get('/timeline', { preHandler: [app.authenticate], schema: { tags: ['dashboard'] } },
  ...
  app.get('/attack-matrix', { preHandler: [app.authenticate], schema: { tags: ['dashboard'] } },
    async () => cached('dash:attack', 60, async () => {
      const { rows } = await pool.query('SELECT * FROM aegis.v_attack_stats');
      return rows;
    }));

  /** GET /api/dashboard/top-sources */
  app.get('/top-sources', { preHandler: [app.authenticate], schema: { tags: ['dashboard'] } },
    async () => cached('dash:sources', 60, async () => {
      const { rows } = await pool.query('SELECT * FROM aegis.v_top_sources LIMIT 15');
      return rows;
    }));
}
